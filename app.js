(() => {
  ...
  function buildPaperHeader({ exam, config, setLabel, includeQr = true, solutionUrl = '' }) {
    const labels = getPaperLabels(exam?.version || 'Bangla');
    const qrPayload = encodeURIComponent(solutionUrl || '');
    const qrCard = includeQr
      ? `<div class="solution-qr solution-qr--header"><div class="solution-qr__title">${labels.qr}</div><img src="https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${qrPayload}" alt="Solution Download QR" /></div>`
      : `<div class="solution-qr solution-qr--header solution-qr--placeholder"><div class="solution-qr__title">${labels.qr}</div><div class="solution-qr__blank"></div></div>`;
    ...
    return `<div class="board-head board-head--${escapeAttr(config.headerTheme || 'classic')}">${headerLogos}<div class="board-head-grid"><div class="board-col board-col--left">${qrCard}<div class="board-col__meta"><p><strong>${labels.version}:</strong> ${escapeHtml(exam.version || 'Bangla')}</p><p><strong>${labels.time}:</strong> ${formatMathForDisplay(timeValue)}</p></div></div><div class="board-col board-col--center"><h1>${formatMathForDisplay(config.headerTitle || 'MegaPrep Examination')}</h1><p><strong>${labels.subject}:</strong> ${formatMathForDisplay(paperSubject)}</p><p><strong>${labels.class}:</strong> ${formatMathForDisplay(config.classLabel || 'N/A')}</p><p><strong>${labels.fullMarks}:</strong> ${formatMathForDisplay(marksValue)}</p><p><strong>${labels.time}:</strong> ${formatMathForDisplay(timeValue)}</p></div><div class="board-col board-col--right"><div class="board-col__spacer"></div><div class="board-col__meta"><p><strong>${labels.subjectCode}:</strong> ${formatMathForDisplay(config.examCode || 'N/A')}</p><p><strong>${labels.setCode}:</strong> ${escapeHtml(setLabel)}</p></div></div></div><p class="instructions">${formatMathForDisplay(config.instructions)}</p></div>`;
  }
