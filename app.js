  ...
  function buildOmrSheetHtml(examId) {
    ...
    const buildDigitColumns = (digits, name) => {
      const columns = Array.from({ length: digits }, (_, colIndex) => {
        ...
        return `<div class="digit-col"><div class="digit-col__head">${colIndex + 1}</div>${bubbles}</div>`;
      }).join('');
      return `<div class="digit-card"><div class="digit-card__title">${name}</div><div class="digit-cols">${columns}</div></div>`;
    };
    for (let setIndex = 0; setIndex < safeSetCount; setIndex += 1) {
