(() => {
  ...
  function parseJsonImportPayload(raw) {
    ...
    const sanitizeInnerQuotes = (text) => {
    ...
    const parseFromString = (text) => {
      try {
        ...
        const cleaned = text
          ...
          .replace(/[‘’]/g, "'")
          .replace(/,\s*([}\]])/g, '$1');
        const normalizedLayout = sanitizeInnerQuotes(normalizeEscapedLayout(cleaned));
        try {
          ...
          try {
            ...
            if (start >= 0 && end > start) {
              ...
              try {
                ...
                const chunks = sliced.match(/\{[\s\S]*?\}(?=\s*\{|\s*$)/g);
                if (chunks?.length) return chunks.map((chunk) => JSON.parse(chunk));
              }
            }
  ...
  function extractImportItems(payload) {
    if (Array.isArray(payload)) {
      return payload.flatMap((entry) => {
        if (entry && typeof entry === 'object' && Array.isArray(entry.questions)) return entry.questions;
        return entry;
      }).filter(Boolean);
    }
    ...
    if (payload.questions === undefined && typeof payload === 'object') return [payload];
    return [];
  }
